...
async function initSubmodules(skipPrompt = false) {
  ...
  if (extraSubmodules.length > 0) {
    prompt.log.warn(`Found ${extraSubmodules.length} submodule(s) not in meta.ts:`)
    for (const path of extraSubmodules) {
      prompt.log.message(`  - ${path}`)
    }
    ...
    if (prompt.isCancel(shouldRemove)) {
  ...
  if (newProjects.length === 0) {
    prompt.log.info('All submodules already initialized')
    return
  }
  ...
  prompt.log.success('Submodules initialized')

  if (existingProjects.length > 0) {
    prompt.log.info(`Already initialized: ${existingProjects.map(project => project.name).join(', ')}`)
  }
}
...
async function syncSubmodules() {
  ...
  for (const [vendorName, config] of Object.entries(VENDORS)) {
    ...
    if (!existsSync(vendorPath)) {
      prompt.log.warn(`Vendor submodule not found: ${vendorName}. Run init first.`)
      continue
    }

    if (!existsSync(vendorSkillsPath)) {
      prompt.log.warn(`No skills directory in vendor/${vendorName}/skills/`)
      continue
    }
    ...
    for (const [sourceSkillName, outputSkillName] of Object.entries(vendorConfig.skills)) {
      ...
      if (!existsSync(sourceSkillPath)) {
        prompt.log.warn(`Skill not found: vendor/${vendorName}/skills/${sourceSkillName}`)
        continue
      }
  ...
  prompt.log.success('All skills synced')
}
...
async function checkUpdates() {
  ...
  if (updates.length === 0) {
    prompt.log.success('All submodules are up to date')
  }
  else {
    prompt.log.info('Updates available:')
    for (const update of updates) {
      prompt.log.message(`  ${update.name} (${update.type}): ${update.behind} commits behind`)
    }
  }
}
...
async function cleanup(skipPrompt = false) {
  ...
  if (extraSkills.length > 0) {
    prompt.log.warn(`Found ${extraSkills.length} skill(s) not in meta.ts:`)
    for (const name of extraSkills) {
      prompt.log.message(`  - skills/${name}`)
    }
  ...
  if (!hasChanges && extraSubmodules.length === 0 && extraSkills.length === 0) {
    prompt.log.success('Everything is clean, no unused submodules or skills found')
  }
  else if (hasChanges) {
    prompt.log.success('Cleanup completed')
  }
}
...
async function main() {
  ...
  if (command === 'init') {
    prompt.intro('Skills Manager - Init')
  ...
  if (command === 'sync') {
    prompt.intro('Skills Manager - Sync')
  ...
  if (command === 'check') {
    prompt.intro('Skills Manager - Check')
    await checkUpdates()
    prompt.outro('Done')
    return
  }
  ...
  if (command === 'cleanup') {
    prompt.intro('Skills Manager - Cleanup')
  ...
  if (skipPrompt) {
    prompt.log.error('Command required when using -y flag')
    prompt.log.info('Available commands: init, sync, check, cleanup')
    process.exit(1)
  }

  prompt.intro('Skills Manager')
